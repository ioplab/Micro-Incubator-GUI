...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=31, verbose=False, dllname='APT.dll'):
        ...
        if not os.path.exists(dllname):
            print( "ERROR: DLL not found. " , dllname )
    ...
    def list_available_devices(self):
        ...
        devices = []
        count = c_long()
        print( "List devices: (Number: hardware type, serial number)" )
        for hwtype in range(100):
            if (self.aptdll.GetNumHWUnitsEx(hwtype, pointer(count)) == 0):
                ...
                if (count.value > 0):
                    ...
                    for ii in range(count.value):
                        if (self.aptdll.GetHWSerialNumEx(hwtype, ii, pointer(serial_number)) == 0):
                            devices.append((hwtype, serial_number.value))
                            print( str( len( devices ) ) + ") " + str( hwtype ) + ", " + str( serial_number.value ) + "." )
        print( "" )
        return devices
    ...
    def initializeHardwareDevice(self):
        ...
        # need some kind of error reporting here
            ...
            print('Connection Failed. Check Serial Number!')
            print( _get_error_text( result) )
    ...
    def setVel(self, maxVel):
        ...
        minVel, oldAcc, oldVel = self.getVelocityParameters()
        self.setVelocityParameters(minVel, oldAcc, maxVel)
        return True
